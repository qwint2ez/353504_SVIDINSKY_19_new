...
from pizza.models import (
    ...
    Ingredient, Allergen, Order, OrderItem
)
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        try:
            # Сначала создаем и применяем миграции
            self.stdout.write('Создание миграций...')
            call_command('makemigrations', 'pizza')
            ...
            ingredients_data = {
                ...
                'Ананасы': {'description': 'Консервированные ананасы', 'is_vegetarian': True},
                'Базилик': {'description': 'Свежий базилик', 'is_vegetarian': True},
                'Оливки': {'description': 'Черные оливки', 'is_vegetarian': True},
                'Пармезан': {'description': 'Сыр пармезан', 'is_vegetarian': True},
                'Горгонзола': {'description': 'Голубой сыр', 'is_vegetarian': True},
                'Бекон': {'description': 'Копченый бекон', 'is_vegetarian': False},
                'Куриное филе': {'description': 'Запеченное куриное филе', 'is_vegetarian': False},
                'Томаты черри': {'description': 'Свежие томаты черри', 'is_vegetarian': True},
                'Красный лук': {'description': 'Свежий красный лук', 'is_vegetarian': True},
                'Перец халапеньо': {'description': 'Острый перец', 'is_vegetarian': True},
                'Говяжий фарш': {'description': 'Жареный говяжий фарш', 'is_vegetarian': False},
                'Артишоки': {'description': 'Маринованные артишоки', 'is_vegetarian': True},
                'Руккола': {'description': 'Свежая руккола', 'is_vegetarian': True},
                'Вяленые томаты': {'description': 'Вяленые томаты в масле', 'is_vegetarian': True},
                'Шампиньоны': {'description': 'Свежие шампиньоны', 'is_vegetarian': True},
                'Лесные грибы': {'description': 'Ассорти лесных грибов', 'is_vegetarian': True},
            ...
            logger.error(f'Ошибка при обновлении базы данных: {str(e)}', exc_info=True)
            self.stdout.write(self.style.ERROR(f'Ошибка при обновлении базы данных: {e}'))
